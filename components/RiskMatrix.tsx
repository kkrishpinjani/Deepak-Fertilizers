...
function cellShade(pr: string, im: string) {
  const score = (pr === "High" ? 2 : pr === "Medium" ? 1 : 0) + (im === "High" ? 2 : im === "Medium" ? 1 : 0);
  if (score >= 3) return "rgba(194,42,42,0.07)";
  if (score >= 2) return "rgba(161,92,0,0.06)";
  return "transparent";
}
...
export default function RiskMatrix({ items }: { items: RiskItem[] }) {
  ...
  return (
    <Stack spacing={2}>
      ...
      {selected && (
        <Card variant="outlined" sx={{ borderLeft: "3px solid", borderLeftColor: `${{ critical: "error", watch: "warning", positive: "success", info: "info", neutral: "text" }[selected.tone]}.main` }}>
          <CardContent>
